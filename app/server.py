...
model_file_url = 'https://drive.google.com/uc?export=download&confirm=UsOg&id=1-eUyal29EVGgNyjcHoowJD6Kk-Urz9ba'
...
async def setup_learner():
    ...
    data_bunch = ImageDataBunch.single_from_classes(
        path, classes, tfms=get_transforms(), size=150).normalize(imagenet_stats)
...
def predict_from_bytes(bytes):
    ...
    result_html = str(result_html1.open().read() +
                      str(predictions[0:3]) + result_html2.open().read())
